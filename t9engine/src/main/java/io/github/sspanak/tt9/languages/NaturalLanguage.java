...
// T9-i-FUTO-patch: ursprungligen "import io.github.sspanak.tt9.preferences.screens.keychars.PreferenceChars2to9;"
// Den klassen är en inställnings-UI-skärm (Compose) vi inte behöver bara för denna
// strängkonstant. Värdet är hämtat direkt från originalfilen.
...
public class NaturalLanguage extends TranscribedLanguage {
	...
	public void updateKeyCharacters(@NonNull SettingsStore settings) {
		for (int key = 2; key <= 9 && key < layout.size(); key++) {
			updateKeyCharacters(
				key,
				settings.getCharsExtra(this, "extra_chars_" + key), // = PreferenceChars2to9.NAME_PREFIX + key
				false
			);
		}
